...
const NestedAccessCheckbox: React.FC<Props> = ({
  ...
  // 🔘 Toggle collapse controller level
  ...
  // 🔘 Toggle satu action / controller
  const toggleAccess = (controller: string, action: string) => {
    const exists = selected.some(
      (x) =>
        x.IdController === controller && x.IdAction === action
    );
  ...
  // 🔘 Toggle 1 controller + semua action di bawahnya
    ...
    if (allChecked) {
      ...
      ids.forEach((id) => {
        if (
          !updated.some(
            (x) =>
              x.IdController === id.IdController &&
              x.IdAction === id.IdAction
          )
        ) {
          updated.push(id);
        }
    ...
    if (allChecked) {
      updated = updated.filter(
        (x) =>
          !ids.some(
            (id) =>
              id.IdController === x.IdController &&
              id.IdAction === x.IdAction
          )
      );
    } else {
      ids.forEach((id) => {
  ...
  return (
    <Box mt={2}>
      ...
      {data.map((menu) => {
        // CHECK IF ALL MENU ITEMS SELECTED
                ...
                {menu.ControllerViewModel.map((ctrl) => {
                  ...
                  return (
                    <Box key={ctrl.IdController} mt={1}>
                      ...
                      {/* ACTION LIST */}
                      <Collapse
                        in={openController.includes(ctrl.IdController)}
                      >
                        <Box ml={4}>
