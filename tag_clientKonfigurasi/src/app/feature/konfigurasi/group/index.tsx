...
const GroupListComponent = () => {
  ...
  const { data, isLoading, mutate } = useSWR(
    ['groups', searchTerm, page, pageSize],
    () =>
      fetchGroups({
        filter: searchTerm,
        page,
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      showSnackbar(err.message || 'Gagal menghapus group', 'error');
    }
  };
  ...
  const handleSaveGroup = async (payload: GroupList) => {
    try {
      ...
      showSnackbar(err.message || 'Gagal menyimpan group', 'error');
    }
  };
    ...
    <Box>
      ...
      >
        ...
        <Stack direction="row" spacing={1}>
          {selectedGroupIds.length > 0 && (
            <AccessButton
              access={{ subject: 'Role', action: 'DeleteRole' }}
              color="error"
              variant="outlined"
      ...
      <TableContainer
        ...
        sx={(theme) => ({
          ...
          overflow: "hidden", // 🔥 WAJIB (fix pojok hitam)
                ...
                <TableRow key={group.Id}>
                  ...
                  <TableCell align="right">
                    <Tooltip title="Edit Group" arrow placement="top">
                      <span>
                        <AccessButton
                          access={{ subject: 'Role', action: 'PutRole' }}
                          type="icon"
                          color="success"
